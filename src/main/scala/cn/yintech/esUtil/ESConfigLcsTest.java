 ...
 * ES集群测试环境配置
 */
public class ESConfigLcsTest {

    private static String hosts = "47.94.79.100"; // 集群地址，多个用,隔开
    ...
    static {
//        credentialsProvider.setCredentials(AuthScope.ANY, new UsernamePasswordCredentials("elastic", "upvo%@ajUW4R")); // 认证信息
    ...
    public static void main(String[] args) throws IOException {
        int num = 0;
        for (String i : searchOnlineAgg("61266", "2020-08-07 23:30:26", "2020-08-07 23:51:05")) {
    ...
    public static List<String> searchOnlineAgg(String extraId, String startTime, String endTime) throws IOException {
        ...
        if (extraId.length() > 0) {
            boolBuilder.must(QueryBuilders.matchQuery("extra_id.keyword", extraId));
        }
...
//        SortBuilder sortBuilder = SortBuilders.fieldSort("@timestamp").order(SortOrder.ASC);// 排训规则
        ...
        // 聚合操作
        AggregationBuilder aggs1 = AggregationBuilders.terms("extra_id_dist").field("extra_id.keyword");
        AggregationBuilder aggs2 = AggregationBuilders.terms("type_count").field("type.keyword");
        AggregationBuilder aggs3 = AggregationBuilders.terms("device_id").field("device_id.keyword").size(1000000000);
        AggregationBuilder aggs4 = AggregationBuilders.terms("uid").field("uid.keyword").size(1000000000);
