 ...
 */
object SparkReadEsPro_yii2_elk_success_bak_20201225 {
        ...
        val value = spark.sparkContext.parallelize(minutes,6)
          .mapPartitions( rdd => {
            rdd.flatMap( v => {
              ...
              esSearchPro_yii2_elk_success1(dt + " " + v._1 , dt + " " + v._2) //  action=app/index
                .union(esSearchPro_yii2_elk_success2(dt + " " + v._1 , dt + " " + v._2)) // action=app/clientGuide
                .union(esSearchPro_yii2_elk_success3(dt + " " + v._1 , dt + " " + v._2)) // action=app/startUpAds
                .union(esSearchPro_yii2_elk_success4(dt + " " + v._1 , dt + " " + v._2)) // action=app/newInfoCircle
                .union(esSearchPro_yii2_elk_success5(dt + " " + v._1 , dt + " " + v._2)) // action=app/tuyereHotList
                .union(esSearchPro_yii2_elk_success6(dt + " " + v._1 , dt + " " + v._2)) // action=app/homePage
                .union(esSearchPro_yii2_elk_success7(dt + " " + v._1 , dt + " " + v._2)) // action=app/indexPlateInfo
            })
          })
          .map(v => {
...
//    val hits = searchResponse.getHits.getHits
  ...
  // action=app/tuyereHotList
  def esSearchPro_yii2_elk_success5( startTime: String, endTime: String): List[String] = {
    val client = ESConfig.client()
    val boolBuilder = QueryBuilders.boolQuery()
    val sourceBuilder = new SearchSourceBuilder()
    //创建一个socket
    ...
    rangeQueryBuilder.lt(endTime)
    boolBuilder.must(rangeQueryBuilder)
    boolBuilder.must(QueryBuilders.matchQuery("action.keyword", "app/tuyereHotList"))
    //    boolBuilder.must(QueryBuilders.nestedQuery("data",QueryBuilders.matchQuery("data.code", "0"))
  ...
  // action=app/homePage
  def esSearchPro_yii2_elk_success6( startTime: String, endTime: String): List[String] = {
    val client = ESConfig.client()
    val boolBuilder = QueryBuilders.boolQuery()
    val sourceBuilder = new SearchSourceBuilder()
    //创建一个socket
    ...
    rangeQueryBuilder.lt(endTime)
    boolBuilder.must(rangeQueryBuilder)
    boolBuilder.must(QueryBuilders.matchQuery("action.keyword", "app/homePage"))
    //    boolBuilder.must(QueryBuilders.nestedQuery("data",QueryBuilders.matchQuery("data.code", "0"))
  ...
  // action=app/indexPlateInfo
  def esSearchPro_yii2_elk_success7( startTime: String, endTime: String): List[String] = {
    val client = ESConfig.client()
    val boolBuilder = QueryBuilders.boolQuery()
    val sourceBuilder = new SearchSourceBuilder()
    //创建一个socket
    ...
    rangeQueryBuilder.lt(endTime)
    boolBuilder.must(rangeQueryBuilder)
    boolBuilder.must(QueryBuilders.matchQuery("action.keyword", "api/indexPlateInfo"))
    //    boolBuilder.must(QueryBuilders.nestedQuery("data",QueryBuilders.matchQuery("data.code", "0"))
